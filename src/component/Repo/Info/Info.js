...
const Details = [
    ...
    { title: "pushed_at", icon: <MdAccessTimeFilled /> },
...
const Info = ({ query, setQuery }) => {
    const { info, loading, error, errorMessage } = useRepoInfoAPI({ query });

    return (
        <>
            {/* if loading then skeleton */}
            {loading && <Skeleton />}
            {/* no loading or error then content */}
            {!loading && !error &&
                        ...
                        <div className="info-detail">
                            ...
                            <div className="info-detail-wrapper">
                                {Details.map((item, index) =>
                                    <div key={index}>
                                        {cloneElement(item.icon, { className: "icon" })}
                                        <span>{info[item.title]}</span>
                                    </div>)}
                            </div>
                        </div>
                    </div>
